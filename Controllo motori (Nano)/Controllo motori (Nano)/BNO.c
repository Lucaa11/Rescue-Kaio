...
uint8_t OPRMode = IMU;// NDOF; //IMU;      			// specify operation mode for sensors
...
double gradi_;
double gradiGiroscopio(int asse){
	if(asse==asseZ){
...
double gradiGiroscopio180(){
	gradi_=readAxis(1);
	if(gradi_<0)gradi_+=360;
	else if(gradi_>=360) gradi_-=360;
	if(gradi_<0)gradi_+=180;
	else if(gradi_>=180) gradi_-=360;
	return gradi_;
}